...
if (isset($_GET['edit'])) {
    $id = $_GET['edit'];
    $sql = "SELECT id_spiele, heim.mannschaft AS heimmannschaft, gast.mannschaft AS gastmannschaft, heimtore, gasttore, zuschauer, datum
...
if(!empty($gruppeauswahl)){
    ...
    $sql = "SELECT id_spiele, heim.mannschaft AS heimmannschaft, gast.mannschaft AS gastmannschaft,  heim.gruppe, 
            spieltag, datum, uhrzeit, spielort_id, arena, kapazitaet, heimtore, gasttore, spiele.zuschauer AS zuschauer
...
if ($stmt = $pdo->query($sql)){
    $spielergebnisse = $stmt->fetchAll(PDO::FETCH_ASSOC);
}


$sql ="SELECT * FROM spielorte
    ORDER BY country, arena, kapazitaet;
";
if ($stmt = $pdo->query($sql)){
