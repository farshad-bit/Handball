...
if (isset($_POST['speichern'])) {
    ...
    If (!empty($_POST['kapazitaet'])) {
        $kapazitaet = $_POST['kapazitaet'];
    } else {
        $err[] = "Die Kapazität der Arena muss angegeben werden.";
    }
    if(!empty($_POST['arena']) and !empty($_POST['country']) and !empty($_POST['kapazitaet'])) {
        $sql = "INSERT INTO spielorte (arena, country, kapazitaet) VALUES (?,?,?)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$arena, $country, $kapazitaet]);
        $meldung_speichern = "Der Spielort $arena in $country wurde mit einer Kapazität von $kapazitaet erfolgreich gespeichert";
    } else {
...
if (isset($_GET['sortk'])) {
    $sortk = $_GET['sortk'];    
    if ($sortk == 1) {
        $order = " ORDER BY kapazitaet ASC";
        $sortk = 2;
    } else {
        $order = " ORDER BY kapazitaet DESC";
        $sortk = 1;
    }
} else {
    $sortk = 1;
